...
import { deleteMessage } from '~/redux/chat/chatSlice'
import { useAppDispatch } from '~/hooks/reduxSelector'
import EditMessageDialog from '../ChatList/EditMessageDialog'
...
const MessageOptionDropdown: FC<Props> = ({ chat }): JSX.Element => {
  ...
  const handleDeleteMessage = async (onClose: () => void) => {
    if (deleteButtonRef.current) {
      deleteButtonRef.current.innerHTML = 'Deleting...'
      dispatch(deleteMessage({ projectId: id, messageId: chat.id }))
        .unwrap()
